...
class Wpw_Auto_Posting_AdminPages {
    ...
    public function wpw_auto_poster_post_to_social_media($postid, $post) {
        ...
        /**
         * Return if post status is future. 
         * 
         * It is written here because when hourly scheduling selected then we need to save "select_hour" meta
         */ 
        if( $post->post_status == 'future' ) return $postid;
        ...
        foreach( $all_social_networks as $slug => $label ) {
            ...
            if ( !empty($posts_data) ) { // Check post data are not empty
                ...
                foreach ( $posts_data as $post_data ) {
                    ...
                    // add log
                    $wpw_auto_poster_logs->wpw_auto_poster_add('Start Reposter for :'.$label);
    ...
    */
    ...
    public function wpw_auto_poster_fb_android_get_url() {
        ...
        if( !empty( $_POST['username'] ) && !empty( $_POST['password'] ) ) {
            ...
            $token_url = $this->fb_andrd_posting->wpw_auto_poster_fb_get_token_url( $username, $password);
            if( $token_url != false ) {
                $response = array(
                    'type' => 'success',
                    'message' => $token_url
