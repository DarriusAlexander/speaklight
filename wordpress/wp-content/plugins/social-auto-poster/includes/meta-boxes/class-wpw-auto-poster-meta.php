...
class Wpw_Auto_Poster_Social_Meta_Box extends Wpw_Auto_Poster_Meta_Box {
	 ...
	 * @access public
  	...
  	public function show_field_tweetpreview( $field, $meta ) {
  		...
  		$tw_tweet_exceed_message = sprintf( __('Twitter only allow %1$s140 characters%2$s limit for the tweet. If the tweet message exceeds the limit it will be automatically truncated.', 'wpwautoposter'), '<strong>', '</strong>');

  		echo "<label for='{$field['id']}' id='{$field['id']}' class='wpw-auto-poster-tweet-preview'>{$meta}</label>";
  		$count = strlen( $meta );
  		$count_class = ( $count > 140 ) ? 'red-color' : '';
