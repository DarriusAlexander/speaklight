...
class Wpw_Auto_Poster_PIN_Posting {
    ...
    public function wpw_auto_poster_pin_initialize() {
        ...
        if ( ( isset ( $_GET['wpw_pinterest_grant'] ) && $_GET['wpw_pinterest_grant'] == 'true' ) 
            ...
            try {
                ...
                $this->logs->wpw_auto_poster_add('Pinterest Exception : ' . $e->getMessage());
                $pinterest = null;
            }
                ...
                try {
                    ...
                    if (!isset($wpw_auto_poster_pin_sess_data[$pin_app_id])) {
                        ...
                        if ($pin_app_id) {
                            ...
                            $this->logs->wpw_auto_poster_add("Sorry, The App Id {$pin_app_id} does not exist.");
                        }
                    ...
                    //record logs exception generated
                    $this->logs->wpw_auto_poster_add('Pinterest Exception : ' . $e->__toString());
    ...
    public function wpw_auto_poster_pin_posting($post) {
        ...
        //record logs for pinterest is enable for posting
    ...
    public function wpw_auto_poster_pin_post_to_userwall($post) {
        ...
        if (!empty($wpw_auto_poster_pin_sess_data)) {
            ...
            if (empty($pin_user_ids)) {
                ...
                $this->logs->wpw_auto_poster_add('Pinterest posting users are not selected.');
            ...
            if (!empty($custom_msg)) {
                ...
                $search_arr = array('{first_name}', '{last_name}', '{title}', '{link}', '{excerpt}', '{sitename}', '%title%', '%link%', '{hashtags}', '{hashcats}');
                $replace_arr = array($first_name, $last_name, $title, $postlink, $excerpt, get_option('blogname'), $title, $postlink, $hashtags, $hashcats);
            ...
            if (!empty($post_to_users)) {
                ...
                foreach ($post_to_users as $post_to) {
                    ...
                    $this->logs->wpw_auto_poster_add('Pinterest auth token for ' . $pin_post_app_id . "|".$pin_post_to_board_id . ' : ' . $auth_token);
                    ...
                    try {
                        ...
                        $this->logs->wpw_auto_poster_add('Pinterest posting exception for ' . $post_to . ' : ' . $e->getMessage());
            ...
            $this->logs->wpw_auto_poster_add('Pinterest grant extended permissions not set.');
