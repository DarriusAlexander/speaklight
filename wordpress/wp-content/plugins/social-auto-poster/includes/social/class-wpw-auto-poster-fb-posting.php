                ...
                try {
                    ...
                    if (!isset($wpw_auto_poster_fb_sess_data[$fb_app_id])) {
                        ...
                        if ($fb_app_id) {
                            ...
                            $this->logs->wpw_auto_poster_add("Sorry, The App Id {$fb_app_id} does not exist.");
                        }
        ...
        if (!empty($wpw_auto_poster_fb_sess_data)) {
            ...
            switch ($post_as) {
                ...
                default:
                    ...
                    $search_arr = array('{title}', '{link}', '{first_name}', '{last_name}', '{sitename}', '{site_name}', '{excerpt}', '{hashtags}', '{hashcats}');
                    $replace_arr = array($post->post_title, $postlink_feed, $first_name, $last_name, get_option('blogname'), get_option('blogname'), $excerpt, $hashtags, $hashcats);
            ...
            if (!empty($post_to_users)) {
                ...
                foreach ($post_to_users as $post_to) {
                    ...
                    if( $facebook_auth_options == 'graph') {
                        ...
                        // Check facebook class is exis or not
                        if (!$facebook) {
                            $this->logs->wpw_auto_poster_add('Facebook not initialized with ' . $fb_post_app_id . ' App.'); // Record logs for facebook not initialized
                            continue;
                        }
                                ...
                                try {
                                    if( $facebook_auth_options == 'graph') {
                                        ...
                                        if(isset($media->error)){
                                            $this->logs->wpw_auto_poster_add('Facebook posting exception for ' . $post_to . ' : ' . $media->error->message );
                                        }

                                        if(!$media){

                                            sap_add_notice( 'Facebook:'.$this->error , 'error');
                                            $this->logs->wpw_auto_poster_add('Facebook posting exception for ' . $post_to . ' : ' . $media->error );
                                        }
                                    }
                            ...
                            try {
                                ...
                                if( $facebook_auth_options == 'graph') {
                                    ...
                                    if ( isset($ret->error) || !$ret ) {
                                        ...
                                        $this->logs->wpw_auto_poster_add('Facebook posting exception for ' . $post_to . ' : ' . $this->error );
