...
class Wpw_Auto_Poster_INS_Posting {
    ...
    public function wpw_auto_poster_ins_posting($post) {
        ...
        // Record logs for instagram is enable for posting
    ...
    public function wpw_auto_poster_ins_post_to_userwall($post) {
        ...
        if (empty($ins_user_ids)) {
            ...
            $this->logs->wpw_auto_poster_add('Instagram posting users are not selected.');
        ...
        if (!empty($custom_msg)) {
            ...
            $search_arr = array('{first_name}', '{last_name}', '{display_name}', '{title}', '{link}', '{excerpt}', '{sitename}', '%title%', '%link%', '{hashtags}', '{hashcats}');
            $replace_arr = array($first_name, $last_name, $display_name, $title, $postlink, $excerpt, get_option('blogname'), $title, $postlink, $hashtags, $hashcats);
            ...
            foreach ($post_to_users as $post_to) {
                ...
                try {
                    ...
                    if( preg_match('/required/i', $e->getMessage() ) ) {
                        $this->logs->wpw_auto_poster_add('Please, go to https://www.instagram.com/, sign in and verify your account.');
                        ...
                        $this->logs->wpw_auto_poster_add('Something went wrong: ' . $e->getMessage());
                ...
                try {
                    if( !empty( $img_path ) ) {
                        ...
                        if ($result->status == 'ok') {
                            ...
                            $this->logs->wpw_auto_poster_add('New Post on instagram with caption ' . $result->media->caption->text);
                            $postflg = true;
                        ...
                        $this->logs->wpw_auto_poster_add(' No media files selected for instgram posting');
